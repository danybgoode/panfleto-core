...
func TestPanfletoMCPPanelRendersWithoutInlineStyleOrScript(t *testing.T) {
	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			...
			// The personalized edition connects with this same token, so the panel points there, and so does
			// the reader's menu in the layout this page renders inside (personalized-edition).
			if n := strings.Count(out, `href="https://editorial-panfleto.vercel.app/tu-edicion/conectar"`); n != 2 {
				t.Errorf("want the edition link in the menu and in the MCP panel, found it %d time(s)", n)
			}
